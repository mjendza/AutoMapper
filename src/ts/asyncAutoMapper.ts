...
export class AsyncAutoMapper extends AutoMapperBase {
    ...
    public createMapForMember(mapping: IMapping, property: ISourceProperty | null): void {
        mapping.async = true;
        mapping.mapItemFunction = (m: IMapping, srcObj: any, dstObj: any, cb: IMapCallback | undefined): void => this.mapItem(m, srcObj, dstObj, cb);
        // property.async = true;
        // property.conversionValuesAndFunctions.push(func);
    }

    public createMapConvertUsing(mapping: IMapping, converterFunction: (ctx: IResolutionContext, cb: IMapCallback | undefined) => void): void {
        mapping.async = true;
        mapping.typeConverterFunction = converterFunction;
        mapping.mapItemFunction = (m: IMapping, srcObj: any, dstObj: any, cb: IMapCallback | undefined): void => this.mapItemUsingTypeConverter(m, srcObj, dstObj, cb);
    }

    public map(m: { [key: string]: IMapping } | undefined, srcKey: string | (new () => any)): (dstKey: string | (new () => any), srcObj: any, cb: IMapCallback) => void;
    public map(m: { [key: string]: IMapping } | undefined, srcKey: string | (new () => any), dstKey: string | (new () => any)): (srcObj: any, cb: IMapCallback) => void;
    public map(m: { [key: string]: IMapping } | undefined, srcKey: string | (new () => any), dstKey?: string | (new () => any), sourceObject?: any): (cb: IMapCallback) => void;
    public map(m: { [key: string]: IMapping } | undefined, srcKey: string | (new () => any), dstKey?: string | (new () => any), sourceObject?: any, cb?: IMapCallback): void;
    ...
    public mapWithMapping(mapping: IMapping, sourceObject: any, callback: IMapCallback | undefined): void {
     ...
     */
    private mapArray(mapping: IMapping, sourceArray: Array<any>, callback: IMapCallback | undefined): void {
        var callbacksToGo = 0;
        ...
        var waitForCallbackToSend = (): void => {
            if (callbacksToGo === 0) {
                callback ? callback(destinationArray) : undefined;
            } else {
                setTimeout((): void => {
    ...
    private mapItemUsingTypeConverter(mapping: IMapping, sourceObject: any, destinationObject: any, callback: IMapCallback | undefined): void {
        var resolutionContext: IResolutionContext = {
            sourceValue: sourceObject,
            destinationValue: destinationObject
        };
        (<(ctx: IResolutionContext, cb: IMapCallback | undefined) => any>mapping.typeConverterFunction)(resolutionContext, callback);
    }
     ...
     */
    private mapItem(mapping: IMapping, sourceObject: any, destinationObject: any, callback: IMapCallback | undefined): void {
        var callbacksToGo = 0;
    ...
    private mapProperty(mapping: IMapping, sourceObject: any, sourceProperty: string, destinationObject: any, callback: IMemberCallback): void {
        super.handleProperty(mapping, sourceObject, sourceProperty, destinationObject,
            (destinationProperty: IDestinationProperty | null, options: IDMCO) => {
    ...
    private transform(
        mapping: IMapping,
        sourceObject: any,
        destinationProperty: IDestinationProperty | null,
    ...
    private processTransformation(
        ...
        switch (transformation.transformationType) {
            ...
            case DestinationTransformationType.MemberOptions: {
                const result = transformation.memberConfigurationOptionsFunc ? transformation.memberConfigurationOptionsFunc(options) : undefined;
            ...
            case DestinationTransformationType.AsyncMemberOptions:
                transformation.asyncMemberConfigurationOptionsFunc ?
            ...
            case DestinationTransformationType.AsyncSourceMemberOptions:
                transformation.asyncSourceMemberConfigurationOptionsFunc ?
